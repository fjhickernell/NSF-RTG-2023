...
\marginpar{\color{magenta}i'll tighten this later.}
...
\begin{comment}
%However, the main computational challenge had remained open to make this theory useful for network and large  data in practice.  
A bulk of my applied work has focused on filling this gap: %e.g.,
%\cfbox{cyan}{
%{\bf Scalability.}
 \cite{GPS} solves the problem of model fitting %}
  for the Holland-Leinhardt $p_1$ model \cite{HL81}, a problem that has been open for decades, based on the theory previously developed in  \cite{PRF10, RPF-mle, SteveAleMe-holland}. 
We build a new methodology and construct  a {dynamic and data-dependent}  MCMC algorithm in statistical software {\tt R}, %analyze some %of the
%  benchmark data sets, show satisfying statistical results, 
 and indicate good mixing time of the chain, some of which were formally proved in a MS thesis I supervised in 2016. **DOMESTIC STUDENT**
%   In 2016, my former Masters student Dillon proved  that this Markov chain algorithm is \emph{rapidly mixing} for many instances. 
% \cite{DiscreteForAlgStatNetworks} surveys how one might construct such good Markov chains by using the graph-theory literature perspective and highlights open problems on extending it to hypergraphs. 
My other recent contributions to statistical analysis and modeling of networks comprises several papers on: 
 more general classes of models for graphs using global summary statistics \cite{coresERGM}; 
 formal models for random hypergraphs \cite{betaHypergraphs}; 
 %\sout{ In \cite{betaHypergraphs} we introduce a model for random hypergraphs in order to represent the occurrence of multi-way interactions among agents in a social network. }
 addressing the well-known issue of degeneracy \cite{dergms};
  and extending the reach of algebraic statistics to testing model fit of mixture models---such as the highly popular latent stochastic blockmodels---using a blend of algebraic and Bayesian techniques \cite{mrc2016}. 
%---.....In contrast,.....---
% there exists a rich body of work by the algebraic geometry community on exploring so-called Markov bases, such as Graver bases [35, 38]. Their structure is known for specific problem classes [34], leading to fast optimization algorithms [36]. Markov bases form a cornerstone of algebraic statistics: [42] translates this concept to polynomial algebra; [46] and others pointed to the computational challenges in using them; and [33, 40, 44, 72, 107] provide fundamental results on Markov bases structure. 

\sout{This proposal provides a unique opportunity to study structured sampling techniques within a broader (nonlinear) optimization context. The iterative nature of the nonlinear optimization algorithms in turn motivates new research into updating Markov bases, and using approximate bases.}
* We will develop metrics and benchmarks rooted in the driving application, and to validate and analyze our theoretical results on mixing times (§3.2) and convergence (§3.1 and §3.3), and benchmark our methods on challenging combinatorial optimization problems.
* We will develop and analyze new hypergraph Markov bases that exploit both problem structure and the iterative nature of our proposed algorithms (§3.2). 
\end{comment}

\subsubsection*{Dynamic networks}\marginpar{\color{magenta}TAG: Hemanshu, Michael}
...
{\color{cyan}please add citations to this part.} \marginpar{[and this text will also likely need to be tightened; entire Theme 2 should be 1.5 pages!]}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\{\color{magenta}update seminar names and theme numbers}
...
1) graduate student seminar \marginpar{\color{magenta}TAG: Hemanshu} - once a month or so, keep it reasonable. 

2) undergraduate research and project proposals \marginpar{\color{magenta}TAG: Despina}  - once a semester or so. 
...
\marginpar{\color{magenta}edit and tighten}
...
Summer is a great time for intense research when faculty and students alike take a break from formal coursework.  Postdoctoral fellows will help RTG faculty to lead our  summer programs. {\color{magenta}HOW??}\marginpar{\color{magenta}TAG: Yuhan. }  
...
%%sonja: 
%PI Petrovi\'c's project  DMS\#1522662, Collaborative Research: Randomized and Structure-Based Algorithms in Commutative Algebra, 8/25/2015-8/25/2018, \$229,577   approaches  problems in computational algebra %the problem of solving systems of polynomial equations, and of finding generators for polynomial ideals 
%using a probabilistic method. %, focusing on providing low expected runtime and the use of random choices -- for computational algebra problems that have high worst case complexity. 
%\\
%\emph{Intellectual merit.} The project uses the underlying combinatorial structure of certain families of problems  %(e.g. polynomial system feasibility) 
%for significant speed-up (from exponential worst-case to expected linear runtimes).
%The resulting algorithms and software can be used in any applied field %commutative algebra, statistics, optimization, graph theory, and other fields 
%where large-scale systems of polynomials arise naturally. %The project adapts to the problem under study a sampling technique that has been used in computational geometry and optimization. 
%% The theoretically expected running time is {\it linear} in the number of input polynomials.
% \\
%\emph{Broader impacts.} There are several applications including statistics and optimization, where key applied methods rely on algorithms to compute such ideal generators. The research will  increase the use of combinatorial structures in polynomial computational problems. 
%Applications exist in graph theory; further applications are expected in combinatorics, coding theory,  systems biology.
%\\ 
%\emph{Products.} The project has so far resulted in 7 research articles (published or submitted and in revision), %1 published article, %(Journal of Symbolic Computation), 
% %3 submitted papers, 
% a software package, several seminar and  conference presentations in less than three years. 


%\subsubsection{Algebraic Statistics 2014} 
...
\bibliography{asi,ref_art,stochas,ml_ref,datasci,VD, quantum,FJHown23,FJH23,LDS,NSF_refs}
